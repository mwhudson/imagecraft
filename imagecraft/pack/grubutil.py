...
def _build_grub_in_chroot(core_prefix: str, rootfs_uuid: str) -> None:
    ...
    early_cfg_path.write_text(
        _CORE_EARLY_CFG_TEMPLATE.format(rootfs_uuid=rootfs_uuid)
    )
    try:
        run(
            "grub-mkimage",
...
def _populate_esp_prime_dir(
    rootfs_prime: Path, esp_prime: Path, rootfs_uuid: str
) -> None:
    ...
    (efi_ubuntu / "grub.cfg").write_text(
        _ESP_GRUB_CFG_STUB_TEMPLATE.format(rootfs_uuid=rootfs_uuid)
    )
